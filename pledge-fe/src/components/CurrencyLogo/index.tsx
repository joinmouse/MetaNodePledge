...
export default function CurrencyLogo({
  ...
  const srcs: string[] = useMemo(() => {
    ...
    if (currency instanceof Token) {
      ...
      return [require(`_assets/images/coins/${currency?.address ?? 'token'}.png`), getTokenLogoURL(currency.address)];
    }
    return [];
  }, [currency, uriLocations]);
  if (currency === ETHER) {
    return <StyledBnbLogo src={require(`_assets/images/coins/bnb.png`)} size={size} style={style} />;
  }
