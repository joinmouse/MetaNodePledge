...
import { AutoColumn, ColumnCenter } from '_components/Column';
import TransactionConfirmationModal, { ConfirmationModalContent } from '_components/TransactionConfirmationModal';
import CurrencyInputPanel from '_components/CurrencyInputPanel';
import DoubleCurrencyLogo from '_components/DoubleLogo';
import { AddRemoveTabs } from '_components/NavigationTabs';
import { MinimalPositionCard } from '_components/PositionCard';
import Row, { RowBetween, RowFlat } from '_components/Row';
import PageUrl from '_constants/pageURL';

import { PairState } from '_src/data/Reserves';
import { useActiveWeb3React } from '_src/hooks';
import { useCurrency } from '_src/hooks/Tokens';
import { ApprovalState, useApproveCallback } from '_src/hooks/useApproveCallback';
import { Field } from '_src/state/mint/actions';
import { useDerivedMintInfo, useMintActionHandlers, useMintState } from '_src/state/mint/hooks';

import { useTransactionAdder } from '_src/state/transactions/hooks';
import { useIsExpertMode, useUserDeadline, useUserSlippageTolerance } from '_src/state/user/hooks';
import { calculateGasMargin, calculateSlippageAmount, getRouterContract } from '_src/utils';
import { maxAmountSpend } from '_src/utils/maxAmountSpend';
  ...
  const handleCurrencyASelect = useCallback(
    (currA: Currency) => {
      ...
      if (newCurrencyIdA === currencyIdB) {
        history.push(PageUrl.Add_Double.replace(':currencyIdA/:currencyIdB', `${currencyIdB}/${currencyIdA}`));
        // history.push(`/add/${currencyIdB}/${currencyIdA}`);
      } else {
        history.push(PageUrl.Add_Double.replace(':currencyIdA/:currencyIdB', `${newCurrencyIdA}/${currencyIdB}`));
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        if (currencyIdB) {
          history.push(PageUrl.Add_Double.replace(':currencyIdA/:currencyIdB', `${currencyIdB}/${newCurrencyIdB}`));
          // history.push(`/add/${currencyIdB}/${newCurrencyIdB}`);
        } else {
          history.push(PageUrl.Add_Single.replace(':currencyIdA/', `${newCurrencyIdB}`));
          // history.push(`/add/${newCurrencyIdB}`);
        }
      } else {
        history.push(
          PageUrl.Add_Double.replace(':currencyIdA/:currencyIdB', `${currencyIdA || 'BNB'}/${newCurrencyIdB}`),
        );
      ...
      <AppBody>
        ...
        <Wrapper style={{ width: '420px' }}>
          ...
          <CardBody>
            <AutoColumn gap="20px">
              {noLiquidity && (
                <ColumnCenter style={{ background: 'rgba(93, 82, 255, 0.1)', borderRadius: '10px' }}>
                  <Pane>
                    <AutoColumn gap="12px">
