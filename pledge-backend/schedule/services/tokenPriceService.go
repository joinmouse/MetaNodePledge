...
func (s *TokenPrice) SavePlgrPrice() {
	...
	if serviceCommon.PlgrAdminPrivateKey == "" {
		log.Logger.Warn("Private key not set, skipping SavePlgrPrice")
		return
	}
...
func (s *TokenPrice) SavePlgrPriceTestNet() {
	// Skip if private key is not set
	if serviceCommon.PlgrAdminPrivateKey == "" {
		log.Logger.Warn("Private key not set, skipping SavePlgrPriceTestNet")
		return
	}
